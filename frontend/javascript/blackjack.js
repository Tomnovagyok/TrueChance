...
function megjelenit() {
    ...
    if (jelenlegiKezEl) {
        if (jatekosKez2.length > 0) {
            jelenlegiKezEl.innerHTML = 'Jelenlegi kéz: ' + (kezIndex === 1 ? 'Első' : 'Második');
        } else {
    ...
    if (osztoOsszegEl)
        osztoOsszegEl.innerHTML =
            'Oszto pont: ' + (osztoRejtett ? '?' : osztoKez.length ? kezErtek(osztoKez) : '-');
}
...
function double() {
    ...
    if (jatekosKez2.length > 0) {
        ...
        if (kezIndex === 1) {
            ...
            if (ertek > 21) {
                var statusEl = document.getElementById('pontjaid');
                if (statusEl) statusEl.innerHTML = 'Állapot: Bust az első keznél - Második kéz';
            } else {
                var statusEl = document.getElementById('pontjaid');
